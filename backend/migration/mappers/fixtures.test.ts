 ...
 *   - `2.5x-authentication-source-a.json`'s `ldap` row: an unsupported 2.x auth provider (no
 *     `backend/modules/authentication/ldap/` directory) — reported, not silently dropped.
...
describe('fixture: 2.5x-authentication-source-{a,b}.json -> mapAuthenticationRows', () => {
  ...
  test('additive (default) policy: domainWhitelist->regex, unsupported ldap, and cross-source displayName disambiguation', async () => {
    ...
    // -- source A: ldap — the unsupported-auth-provider path this task names: no 3.0 `ldap` module
    // directory, reported rather than dropped silently, no row written.
    assert.equal(resultA.results[2].status, 'unsupported')
    assert.equal(resultA.results[2].sourceKey, 'ldap')
    assert.equal(resultA.results[2].module, 'ldap')
    assert.equal(resultA.results[2].row, undefined)
    assert.match(resultA.results[2].message!, /ldap/)
