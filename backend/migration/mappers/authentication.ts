...
export function mapAuthenticationRow(
  ...
  if (!MODULES_WITH_VERIFIED_CONFIG_MAPPING.has(module) && hasNonEmptyConfig(row.config)) {
    return {
      ...
      module,
      status: 'flagged',
      message: `module '${module}' has no verified 3.0 config mapping — its 2.x config was not translated and would otherwise have been silently discarded; see docs/migration/2.5x-settings-auth-storage-field-mapping.md's Part 2 provider inventory`
    }
  }
