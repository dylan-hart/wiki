 ...
 * site-config cache, and `CARDINAL.data.systemIds`. A worker thread's `CARDINAL` (`worker.ts`) carries none of
 ...
 * locale mis-resolution to `'en'`), invisible to `tsc` because the worker's `CARDINAL` is typed as the same
 * full `CardinalGlobal` the main process populates. Replicating that much of boot inside a worker just to
