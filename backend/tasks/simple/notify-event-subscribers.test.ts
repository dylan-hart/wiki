...
describe('notify-event-subscribers task', () => {
  ...
  test('an empty subscriberIds list is a no-op', async () => {
    await notifyEventSubscribers(payload({ subscriberIds: [] }))
    assert.equal(sendEventSubscriptionNotification.mock.calls.length, 0)
  })

  test('sends one notification per subscriber, to their resolved email', async () => {
    await notifyEventSubscribers(payload({ subscriberIds: ['user-a', 'user-b'] }))
  ...
  test('passes the event through to the mail call', async () => {
    await notifyEventSubscribers(payload({ event: 'comment:new', subscriberIds: ['user-a'] }))
  ...
  test('a subscriber with no email address is skipped, with a warning, and does not stop the rest', async () => {
    ...
    await notifyEventSubscribers(payload({ subscriberIds: ['no-email', 'has-email'] }))
  ...
  test('one subscriber whose send fails does not stop a second subscriber from being sent to', async () => {
    ...
    await notifyEventSubscribers(payload({ subscriberIds: ['fails', 'succeeds'] }))
