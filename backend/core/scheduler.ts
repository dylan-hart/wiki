...
export default {
  ...
  async init() {
    ...
    const poolOptions = {
      ...
      exitHandler: () => CARDINAL.logger.debug('worker', 'worker offline'),
      onlineHandler: () => CARDINAL.logger.debug('worker', 'worker online'),
      ...
      //    `CARDINAL.capabilities`, and a worker thread never calls `syncSchemas()` itself to learn it
