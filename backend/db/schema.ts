...
export const pageEditSubmissionApprovals = pgTable(
  ...
  (table) => [
    // -> Covers lookups by submission as well, being the leading column
...
export const userGroups = pgTable(
  ...
  (table) => [
    // -> (userId, groupId) is covered by the primary key below, which also serves a lookup by
    //    userId alone (its leading column) -- no separate index needed for either.
    primaryKey({ columns: [table.userId, table.groupId] }),
    // -> groupId is the PK's non-leading column, so it needs its own index -- genuinely used by
