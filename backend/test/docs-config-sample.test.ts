...
describe('config.sample.yml documentation header (#1976)', () => {
  ...
  test('documents the pool tunables and their effective defaults', async () => {
    ...
    // -> The real effective default: base.yml explicitly sets min: 1, max: 20 (WP #2276 sized
    //    `max` at double node-postgres's own unmetered default of 10). The doc must state base.yml's
    //    actual default, not the pre-#2276 pg-library fallback.
    assert.ok(
      /\b20\b/.test(sample) && /base\.yml/i.test(sample),
      "expected config.sample.yml to state that base.yml's default of 20 applies to `pool.max`"
    )
  })
