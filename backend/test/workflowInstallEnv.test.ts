 ...
 * fail a job on a nuget outage. `ONNXRUNTIME_NODE_INSTALL=skip` (its non-deprecated switch;
 * `ONNXRUNTIME_NODE_INSTALL_CUDA=skip` is marked deprecated) exits that postinstall early, and the
 * CPU binding ships inside the tarball, so skipping loses nothing the semantic-search path needs.
 *
 * The switch lives in each install step's own `env:` -- not a job- or workflow-level one -- so the
 * check is per install step, and a newly added backend install step without it fails here rather
 * than on a nuget outage.
 *
