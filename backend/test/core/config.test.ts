...
/**
 * Regression test for a pre-existing bug in `config.init()`'s DB_PASS_FILE (Docker secret) handling:
 * `.trim()` was called on the *Promise* returned by `fs.readFile(...)` (masked by an `as any` cast)
 * rather than on the resolved string, so every read threw `promise.trim is not a function` and
 * DB_PASS_FILE never worked — the `catch` always ran and `process.exit(1)` killed the process. Fixed
 * by awaiting the read before calling `.trim()`.
 *
