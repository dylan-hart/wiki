...
- `core/` — long-lived singletons: `config.ts` (yml + db-backed settings), `db.ts` (pg pool, Drizzle
  ...
  - `core/http/` — everything `index.ts` used to do to a Fastify instance: `server.ts`
    ...
    `CARDINAL.app`/`CARDINAL.server`; plus `registerStaticAssets(app)`), `security.ts` (helmet/CSP/CORS),
...
- `core/scheduler.ts` → `path.join(CARDINAL.SERVERPATH, 'worker.ts')` (the poolifier pool entry)
...
CARDINAL.logger.error('jobs', 'purgeUploads failed, no attempts left', { job: job.id, attempts: 3, error: err })
