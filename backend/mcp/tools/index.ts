 ...
 * helper the others lean on), write (create/update a page, upload an asset), and the diagram renderer
 * (site-independent — it draws from posted source, not from any page). Every tool is registered
 * regardless of what `getCtx()` grants — `create_page`/`update_page`/`upload_asset` refuse at call time
 * for anything but a personal access token (`pageActorFor()`/`ctx.userId` in `mcp/auth.ts`), the same
 * way the read tools refuse per page rather than being hidden from a caller who cannot use them.
 *
