 ...
 * helper the others lean on), write (create/update a page, delete an asset), and the diagram renderer
 * (site-independent — it draws from posted source, not from any page). Every tool is registered
 * regardless of what `getCtx()` grants — `create_page`/`update_page` refuse at call time for anything
 * but a personal access token (`pageActorFor()` in `mcp/auth.ts`), `delete_asset` refuses at call time
 * for anything lacking `manage:assets` on the asset's folder, the same way the read tools refuse per
 * page rather than being hidden from a caller who cannot use them.
 *
