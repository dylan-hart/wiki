 ...
 * helper the others lean on), write (create/update a page, rename an asset), and the diagram renderer
 ...
 * but a personal access token (`pageActorFor()` in `mcp/auth.ts`), the same way the read tools refuse
 * per page rather than being hidden from a caller who cannot use them. `rename_asset` is the one write
 * tool with no such restriction — see its own doc comment.
 *
