 ...
 * - `id` and `hash` have no destination here. This writer mints its own UUID (shared with the paired
 *   `tree` row, exactly like a live upload) and computes its own hash from the resolved path, exactly
 *   as `WIKI.models.tree.addAsset` already does for an upload.
...
export interface AssetImportSummary {
  /** How many assets this summary has recorded a write for. */
  written: number
...
export function createAssetImportSummary(): AssetImportSummary {
  return { written: 0, authorFallbacks: [] }
}
