...
type AiGenerate = (
  siteId: string,
  prompt: string,
  context?: { system?: string; maxOutputTokens?: number; signal?: AbortSignal }
) => Promise<string | null>

export function aiRegistry(): { generate: AiGenerate } | null {
  const ai = (CARDINAL.models as unknown as { ai?: { generate?: AiGenerate } }).ai
  return typeof ai?.generate === 'function' ? (ai as { generate: AiGenerate }) : null
}
