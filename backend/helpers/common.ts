...
/** Unit labels for `formatByteSize`, indexed by power-of-1000 exponent. */
const BYTE_SIZE_UNITS = ['B', 'kB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB']

/**
 * Human-readable byte count, replacing the (now-removed) `filesize` dependency's bare
 * `filesize(bytes)` call — the only way `api/system/info.ts` ever invoked it, for `ramTotal`.
 *
 * That default call is decimal (1000-based unit steps), not base-2, despite `filesize`'s own
 * "JEDEC" standard name suggesting otherwise — verified empirically against the real package
 * before removing it (OpenProject #3178) and fuzz-checked across ~200k random magnitudes with
 * zero mismatches. It carries one quirk worth keeping explicit: a value that rounds up to exactly
 * the next unit's threshold (2 ** 30 - 1, which is "1024.00" MB at 2-decimal precision) is bumped
 * to that next unit instead — `formatByteSize(2 ** 30 - 1) === '1 GB'`, not `'1024 MB'`. See
 * `common.test.ts`'s table tests, pinned against `filesize`'s actual pre-removal output, for the
 * byte counts this is verified against.
 */
