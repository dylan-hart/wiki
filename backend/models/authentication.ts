...
export interface AuthFlow {
  ...
  codeVerifier: string
  /**
   * The `ID` of the outbound AuthnRequest, present only for SAML — no other protocol here has
   * anything of its own for `validateInResponseTo` to check. Generated by `authorizationUrl()`
   * (returned to the caller, which is what writes it onto `req.session.authFlow`), and handed back on
   * `AuthFlowCallback` so `profile()` can refuse a `SAMLResponse` whose `InResponseTo` is not the one
   * this session actually sent — an unsolicited or replayed assertion, not an answer to this login.
   */
  requestId?: string
}
