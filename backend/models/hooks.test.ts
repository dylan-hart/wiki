 ...
 * whether a subscribed user gets queued a `notifyEventSubscribers` job, an unsubscribed one does not,
 * several subscribers are batched into a single job rather than one job each, and the webhook-queued
 * return value stays exactly what it was before this feature existed either way.
 */
describe('Hooks.emit event-subscriber fan-out (unit)', () => {
  ...
  test('an event with a subscriber queues a batched notifyEventSubscribers job carrying their id', async () => {
    subscribers = ['user-subscribed']

    await hooksModule.hooks.emit('page:edit', 'site-1', { id: 'page-1' })

    const notifyJobs = queuedJobs.filter((job) => job.task === 'notifyEventSubscribers')
  ...
  test('an event with no subscribers queues no notifyEventSubscribers job', async () => {
    subscribers = []
 ...
 * `tasks/simple/notify-event-subscribers.test.ts` covers the next step, the queued job actually
 * resulting in a sent email, with the model layer mocked instead.
 */
describe(
  ...
  () => {
    ...
    test('a subscribed user is queued a notification; an unsubscribed user is not', async () => {
      ...
      const notifyJobs = addJob.mock.calls
        ...
        .filter((job) => job.task === 'notifyEventSubscribers')
      assert.equal(notifyJobs.length, 1)
