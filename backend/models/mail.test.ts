...
describe('mail template senders', () => {
  ...
  test('sendRegistrationCollision notifies the existing owner without leaking a token or link', async () => {
    await mail.sendRegistrationCollision({ to: 'fixture@example.com', name: 'Fixture User' })
    assert.equal(sendCalls.length, 1)
    const msg = sendCalls[0]
