...
class MailModel {
  ...
  /**
   * Minimal, generic notification sent by `tasks/simple/notify-event-subscribers.ts` for one user
   * subscribed to an event (`models/eventSubscriptions.ts`), whenever `models/hooks.ts#emit()` fires
   * it — the per-user counterpart to the site-configured webhook `hooks` deliver instead. Deliberately
