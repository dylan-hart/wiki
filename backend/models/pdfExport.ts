...
export interface PdfExportRequest {
   ...
   * page they may. The API route requires a logged-in actor before this is ever called (OpenProject
   * #2258/#2262), so `null`/absent here means a personal-access-token caller (no session cookie to
   * forward), not an anonymous one.
   */
