...
class ApiKeys {
  ...
  async regenerateCertificates(): Promise<number | null> {
    ...
    //    rather than going on trusting tokens this one has just disowned. Needs no restart: `verify()`
    //    below reads `WIKI.config.auth.certs.public` fresh on every call rather than a value handed to
    //    a plugin at boot, so `reloadConfig`'s `loadFromDb()` is enough on its own — the same pattern
    //    `models/sessions.ts`'s `signer` now follows for the session secret. Verified live across a
    //    real two-instance setup for task 589 — a second instance picked up the new `generatedAt`
    //    within a second of this call, with no restart.
