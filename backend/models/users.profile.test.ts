 ...
 */
/**
 * Feature #2425: `getNotificationSubscriptions` / `setNotificationSubscriptions`, the per-user,
 * per-event-type email opt-in stored under `prefs.eventSubscriptions`. DB-backed for the same reason
 * `updateProfile` above is -- this round-trips through `getById()` / `updateUser()`, not just a pure
 * merge function.
 */
