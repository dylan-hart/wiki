 ...
 * the secret off, and a row is scoped to its own site. `createCredential`/`updateAllowedOrigins`'s own
 * origin-plus-path-prefix validation (OpenProject #2195) is exercised here too, alongside the DB
 * round trip, since both throw before ever touching the database.
 */
describe('blockCredentials (DB-backed)', { skip: !hasTestDatabase() }, () => {
  ...
  test('createCredential stores every given origin, in order, no dedup applied at this layer', async () => {
    const created = await blockCredentials.createCredential(
      ...
      'Multi-origin',
      'secret',
      ['https://api.example.com', 'https://internal.example.com/v1']
    )
  ...
  test('createCredential rejects a bare hostname with no scheme', async () => {
    await assert.rejects(
      blockCredentials.createCredential(fixtures.siteId, 'Bad', 'secret', ['api.example.com']),
      (err: any) => {
  ...
  test('createCredential rejects a non-http(s) scheme', async () => {
    await assert.rejects(
      blockCredentials.createCredential(fixtures.siteId, 'Bad', 'secret', [
        'ftp://api.example.com'
      ]),
      (err: any) => {
        assert.equal(err.statusCode, 400)
  ...
  test('createCredential rejects a pattern carrying a query string', async () => {
    await assert.rejects(
      blockCredentials.createCredential(fixtures.siteId, 'Bad', 'secret', [
  ...
  test('createCredential rejects a pattern carrying a fragment', async () => {
    await assert.rejects(
      blockCredentials.createCredential(fixtures.siteId, 'Bad', 'secret', [
  ...
  test('updateAllowedOrigins rejects the same malformed entries createCredential does', async () => {
    const created = await blockCredentials.createCredential(fixtures.siteId, 'Guarded', 'secret', [
      'https://api.example.com'
    ])
    await assert.rejects(
      blockCredentials.updateAllowedOrigins(fixtures.siteId, created.id, ['api.example.com']),
      (err: any) => {
        assert.equal(err.statusCode, 400)
        return true
      }
    )
    // -> The rejected call never reached the database -- the stored list is unchanged.
    const resolved = await blockCredentials.getCredentialForResolve(fixtures.siteId, created.id)
    assert.deepEqual(resolved?.allowedOrigins, ['https://api.example.com'])
  })
