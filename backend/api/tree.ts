...
async function routes(app: FastifyInstance) {
  ...
  app.get<{ Params: { siteId: string; folderId: string } }>(
    ...
    async (req, reply) => {
      const folder = await WIKI.models.tree.getFolderById(req.params.siteId, req.params.folderId)
      if (!folder) {
        return reply.notFound('This folder does not exist.')
      }
  ...
  app.post<{ Params: { siteId: string }; Body: FolderBody }>(
    ...
    async (req, reply) => {
      /*
        ...
        `parentId` the parent has to be looked up, and a missing one -- including a `parentId`
        belonging to another site, which `getFolderById` is scoped by siteId so it never matches --
        is a 404 here rather than being handed to the model.
      */
      let parentPath = req.body.parentPath ?? ''
      let parent: Awaited<ReturnType<typeof WIKI.models.tree.getFolderById>> = null
      if (req.body.parentId) {
        parent = await WIKI.models.tree.getFolderById(req.params.siteId, req.body.parentId)
        if (!parent) {
          return reply.notFound('This folder does not exist.')
        }
  ...
  app.patch<{ Params: { siteId: string; folderId: string }; Body: FolderBody }>(
    ...
    async (req, reply) => {
      const existing = await WIKI.models.tree.getFolderById(req.params.siteId, req.params.folderId)
      if (!existing) {
        return reply.notFound('This folder does not exist.')
      }
      if (
  ...
  app.delete<{ Params: { siteId: string; folderId: string } }>(
    ...
    async (req, reply) => {
      ...
      const removed = await WIKI.models.tree.deleteFolder(req.params.siteId, req.params.folderId)
