...
before(async () => {
  ;(globalThis as any).WIKI = {
    ...
    models: {
      tree: {
        ...
        getTree: async () => [],
        // -> DELETE FOLDER and PATCH FOLDER's shared default: no descendants, nothing to authorize.
        //    Tests covering OpenProject #2100 / #2102 override this per-test.
  ...
  //    route's 4xx entries reference requires. Needed here because DELETE/PATCH FOLDER's #2100/#2102
  //    tests are the first in this file to actually exercise a 4xx response.
...
test('DELETE FOLDER route: refused 403 when a descendant asset fails manage:assets, deleting nothing', async () => {
  ...
  ;(globalThis as any).WIKI.models.tree.listDescendants = async () => ({
    pages: [],
    assets: [{ path: 'sub/file.png', locale: 'en' }]
  })
  ;(globalThis as any).WIKI.models.tree.deleteFolder = async (folderId: string) => {
    deleteFolderCalls.push(folderId)
    return { pages: [], assets: [] }
