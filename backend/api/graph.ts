 ...
 * data and every response is still narrowed to the actual caller's own `read:pages` grants by
 * `assembleGraph` below, regardless of who happened to warm the cache.
 */
...
async function routes(app: FastifyInstance) {
  ...
  app.get<{ Params: { siteId: string } }>(
    ...
    async (req, reply) => {
      ...
      const data = await loadGraphData(req.params.siteId, req.session?.authenticated === true)
      if (!data) {
