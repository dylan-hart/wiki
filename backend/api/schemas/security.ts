...
export async function registerSchemas(app: FastifyInstance): Promise<void> {
  app.addSchema({
    ...
    properties: {
      ...
      trustProxy: {
        // -> A union combinator rather than `type: ['boolean', 'string']`: AJV's strict mode only
        //    allows a type array of one real type plus `'null'`.
        //    `anyOf`, not `oneOf`: Fastify's default AJV sets `coerceTypes: 'array'`, under which a
        //    real boolean also coerces into the `string` branch, so `oneOf` (exactly one match)
        //    rejects it. `anyOf` stops at the first match, so `boolean` must stay listed first.
