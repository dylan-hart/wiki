 ...
 * System API Routes
 *
 * One resource, seven sub-plugins by responsibility (API-F5): what the instance IS (`info.ts`), what
 ...
 * exporting a whole-instance replication snapshot (`replicationExport.ts` -- Epic #2437's source
 * side, WP #2489), and wiping the whole instance and replacing it with another instance's snapshot
 * (`replication.ts` -- Epic #2437's target side, WP #2490). The replication pair is a deliberately
 * separate archive/feature from `transfer.ts`'s per-site export/import, not folded into it. Every
 * route path, method, schema, `tags` and `config.permissions` is what the single `api/system.ts`
 * declared, and each sub-plugin registers unprefixed, so the mounted table under `/system` is
 * identical.
 *
