 ...
 * (`replication.ts` -- the target side, WP #2490). The export/import pair is a deliberately separate
 * archive/feature from `transfer.ts`'s per-site export/import, not a sixth responsibility folded
 * into it. Every route path, method, schema, `tags` and `config.permissions` is what the single
