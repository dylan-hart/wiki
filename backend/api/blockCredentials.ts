...
import { ORIGIN_PREFIX_PATTERN_SOURCE } from '../helpers/network.ts'
      ...
      schema: {
        ...
        body: {
          ...
          properties: {
            ...
            allowedOrigins: {
              type: 'array',
              items: { type: 'string', minLength: 1, pattern: ORIGIN_PREFIX_PATTERN_SOURCE },
              minItems: 1,
              description:
                "Origins (scheme + host + optional port), each with an optional path prefix, this credential's secret may be sent to -- e.g. `https://api.example.com/v1`. At least one is required."
            }
          }
        },
        response: {
        ...
        body: {
          ...
          properties: {
            allowedOrigins: {
              ...
              items: { type: 'string', minLength: 1, pattern: ORIGIN_PREFIX_PATTERN_SOURCE },
              description:
                "Origins (scheme + host + optional port), each with an optional path prefix, this credential's secret may be sent to. May be empty."
            }
          }
        },
        response: {
