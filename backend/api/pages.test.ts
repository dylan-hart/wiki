 ...
 * `POST …/classification-conflicts/resolve` just above -- that route refuses the WHOLE request on
 * the first page the caller may not act on; this one reports that one page as `skipped` and keeps
 * going, since a bulk action starts from an arbitrary admin-picked selection rather than a
