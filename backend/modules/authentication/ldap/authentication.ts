 ...
 * apart from the outside is an account-enumeration oracle. The one exception is the *administrative*
 * bind (`bindDn`/`bindCredentials`) failing, which is not a login mistake — it means the strategy itself
 * is misconfigured, so it is reported as `ERR_STRATEGY_MISCONFIGURED` instead.
 */
