...
| `list_watched_pages`        | The caller's own watch list on a site, most recently watched first. **Requires a personal access token** — the watch list belongs to its owner, and an admin-issued key has no user to list one for.                                                                                                                    |
| `list_page_watchers`        | Who is watching a wiki page, oldest watcher first, with the total across all of them. Readable by anybody who may read the page, signed in or not.                                                                                                                                                                      |
...
`unwatch_page`/`set_page_watch_preference` instead refuse **at call time** with a clear error when
...
`rename_asset` refuse the same way for a token missing `manage:assets` on the asset's folder.
