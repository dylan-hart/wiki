// Drift check for the "## TODO/FIXME audit" section of docs/variances.md (task #780).
//
// docs/todo-fixme-audit.test.mjs (an earlier pass of this same task) locks down specific, durable
// outcomes of the audit -- the stale WIKI.sites comment fix, the bucket references, etc. -- but it
// hard-codes what it checks, so it cannot notice a marker the audit never looked at in the first
// place. This test is the "re-run the grep before trusting this list" instruction the audit's own
// text gives, automated: it walks backend/ and frontend/src/ for TODO/FIXME markers and fails if
// any file carrying one isn't named anywhere in the audit section.
//
// Deliberately checks by file path, not file:line: line numbers shift on unrelated edits to the
// same file, which would make this test flake on changes this task doesn't care about. A new
// marker landing in a file the audit already discusses still wants a human read, but re-flagging
// every whitespace-shifted line number would be noise, not signal.
//
// Run directly:
//   node --test docs/todo-fixme-drift.test.mjs
...
const variancesPath = path.join(docsDir, 'variances.md')
