...
**Reversed 2026-08-31 (OpenProject #1958):** `isSelectable()` no longer treats `codeTemplate` as an
independent grant — it now gates on `hasImplementation` alone, and Disqus/Commento/Artalk's
`definition.yml`s now declare `isAvailable: false`. The 2026-08-18 decision above was made on the
premise that a future change would render a `codeTemplate` provider's embed on page views; nothing
ever did (`docs/audit-2026-08-24/product-value.md` §14 caught the resulting picker offering three
providers that don't actually render comments), and OpenProject #1953/#1958/#1962 confirmed building
that render path was materially larger than the config flip it looked like — it needs a new public,
per-page-permission-gated API to expose the active provider to anonymous page readers (nothing today
exposes one), plus vendor-specific glue for three distinct third-party SDKs, for a "product-honesty
wart" the audit itself called low severity. `AdminComments.vue` already rendered an unavailable
provider as a disabled row, so marking these three unavailable needed no further UI work. The
`codeTemplate` field itself, and the permission/canonical-URL boundary doc comments on
`CommentProviders`, are left in place as design-intent documentation rather than stripped — they no
longer gate anything, but nothing in this reversal contradicts them, and a real embed integration
would still have to honor both if it were ever built.
