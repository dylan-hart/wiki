...
- **`unsupported-auth-provider`** — a 2.x `users` row, or a 2.x `authentication` (strategy) row, whose
  ...
  `dropbox`, `facebook`, `firebase` (`backend/migration/report.ts`'s
  ...
  `rocketchat`, `saml`, `slack`, `twitch` — so a 2.x user or strategy on
  `ldap`/`saml`/`cas`/`auth0`/`okta`/`rocketchat` is **not** in this unmappable bucket any more; those
  six providers do have a 3.0 module. A user/strategy row
