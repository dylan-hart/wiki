...
| Facebook                        | OAuth2-only | `facebook` module | Plain OAuth2 against the Graph API, no ID token. Built as a thin `OAuth2Authentication` subclass (Task 3262), same shape as `discord/authentication.ts`, fixing Graph API v19 endpoints (`/v19.0/dialog/oauth`, `/v19.0/oauth/access_token`, `/me?fields=id,name,email`) and the `email public_profile` scope.                                                                                                                                                                                               |
| Dropbox                         | OAuth2-only | Deferred          | Plain OAuth2, no ID token — same rationale as Facebook; not named as a Feature 355 deliverable.                                                                                                                                                                                                                                                                                                                                                                                                              |
| RocketChat                      | OAuth2-only | `oauth2` preset   | Plain OAuth2 against a self-hosted Rocket.Chat instance, no ID token — re-confirmed for Task #3263 against current Rocket.Chat docs: the OAuth App resource (`developer.rocket.chat/apidocs/get-oauth-app`) is `_id`/`name`/`active`/`clientId`/`clientSecret`/`redirectUri` plus an Authorization/Access Token URL pair, with no discovery document, JWKS or ID token mentioned anywhere in that surface. Built as `backend/modules/authentication/rocketchat/`, self-hosted like `keycloak`'s `serverUrl`. |
