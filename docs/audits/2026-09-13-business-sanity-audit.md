...
| **CAS auth is decorative** | `docs/variances.md:450` — CAS 1.0 only, **cannot provision or log in any account** | Worse than not offering CAS at all: it implies a capability that silently fails. An evaluator who tries it loses trust in every other claim in the docs. |
...
| **Several auth presets never verified against live providers** | Auth0, Okta, Keycloak, GitLab, Twitch, Discord, Slack OIDC presets are unit-tested against mocks only (`docs/variances.md` Tasks 437/438/440) | A prospective evaluator's first login attempt is a bad place to discover a live-integration bug. |
