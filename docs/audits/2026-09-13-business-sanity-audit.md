...
| **No mobile app / offline reading** | No native app and no offline reading. An installable PWA shell (per-site web manifest, no-op service worker) now exists; see `docs/decisions/2026-09-21-pwa-shell-only.md` | The installable shell is the only piece built; the offline-reading gap is unchanged. Every competitor above at least offers a usable mobile web experience; several ship native apps. |
