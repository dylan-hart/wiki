...
**Date:** 2026-08-26
**Status:** Decided — executed in the same change (WP #2171/#2175)
**Author:** WP #2175 (also settling WP #2171, its sibling decision task, since no separate ADR
existed yet), part of #2154, Epic #1360 "Security: content injection, XSS & untrusted file serving"
...
1. **Three stored fields — `jsLoad`/`jsUnload`/`css` (API: `scriptJsLoad`/`scriptJsUnload`/
   ...
   (`siteStore.theme.injectCSS`/`injectHead`/`injectBody`) is the _site_ theme's own, unrelated
...
2. **The permission names `write:scripts`/`write:styles` themselves** — checked in FOUR places in
   `backend/models/pages.ts` (`createPage`, `updatePage`, `queueRerender`, _and_ `buildScripts`),
   ...
   sites in the same file). This is a core, currently-correct piece of _this very epic's_ subject
...
- **This branch's own standing policy is unambiguous for a half-present feature.** CLAUDE.md:
  ...
  and _zero_ lines of code anywhere that ever read them back out and do anything with them. That is
...
- **Implementing execution for these three fields specifically would be a bigger, riskier feature
  than the fields alone suggest**, and would additionally _conflict_ with the sanitizer's existing,
...
- **The feature has no independent product case in front of it.** No linked Feature/Epic asks for
  a _second_, dedicated per-page-load/unload script mechanism; the fields exist because a column,
