...
/*
  ...
  exclusion of `FLAKY_GLOB` and select the lane instead. Everything else (plugins, aliases, the
  lightningcss nesting downlevel, `test/setup.js`) comes from `vitest.config.js` unchanged, so a lane
  test runs in exactly the environment it would have run in before it was quarantined.
