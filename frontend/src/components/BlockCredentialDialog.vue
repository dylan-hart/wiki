...
/**
 * Matches `isValidOriginPrefixPattern`'s own accepted syntax (see `helpers/originPattern.js`)
 * rather than accepting anything non-empty (OpenProject #2195/#2198): a malformed entry used to be
 * stored silently and just never match any real request at resolve time.
 */
...
function addOrigin() {
  ...
  //    The push decision itself is `isValidOriginPrefixPattern(value)` directly, not
  ...
  if (!isValidOriginPrefixPattern(value)) {
    return
  }
  state.originInput = ''
