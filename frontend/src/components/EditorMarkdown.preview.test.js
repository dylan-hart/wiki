...
/*
  With no preview pane open, a debounced flush skips the renderer rather than running the full
  markdown-it + KaTeX + highlight.js pipeline over the whole document and discarding the result. It
  still syncs `pageStore.content`, so a save never reads stale text; reopening the pane catches up
  the pending render; and the save-path flusher renders a stale document before `pageSave()` reads
  `render`.
*/
describe('EditorMarkdown skips rendering while the preview pane is closed (OpenProject #1889)', () => {
  ...
  it('does not run the renderer on a debounced edit while the pane is closed', async () => {
    ...
    // -> `md` is only assigned once `onMounted` resolves
    ...
    // -> The registered debounced wrapper; calling it arms the 500ms debounce
  ...
  it('the save-path flusher renders a stale document before pageSave reads pageStore.render', async () => {
    ...
    // -> What `pageSave()` calls synchronously before it reads `render`: the flusher syncs `content`
    //    from the live editor value and, because the pane is closed, catches up the render too --
    //    unlike the plain debounced flush the first test above covers.
...
/*
  `previewShown` starts `false` unconditionally and opens only once `previewWidth` is resolved from
  the async settings fetch, so the pane's one entrance per mount is at the reader's own width from
  its first frame rather than appearing at the stylesheet's fallback and snapping. That entrance uses
  a distinct, faster transition, which is what `previewEverRevealed` is for: it must not reset, or a
  later toggle would borrow it instead of the slower toggle-button transition.
*/
describe('EditorMarkdown preview pane initial reveal (OpenProject #809 follow-up)', () => {
  ...
  it('marks the pane as having revealed once mount settles, and keeps it marked across a later toggle', async () => {
    ...
    // -> The one entrance this mount has already happened by the time `mountEditor` returns: it
    //    awaits `flushPromises`, which settles the `nextTick` this flag is set in.
    expect(wrapper.vm.previewEverRevealed).toBe(true)
    expect(wrapper.find('.editor-markdown-preview').exists()).toBe(true)

    // -> The toggle-button path, which the slower `editor-markdown-preview` transition owns: the
    //    flag must stay set, or a later reshow would be drawn as a first entrance.
    const hideButton = wrapper
      .findAllComponents(WBtn)
      .find((candidate) => candidate.props('icon') === 'tabler:eye-off')
    await hideButton.trigger('click')
  ...
  it("uses App.vue's prefetched settings instead of fetching again, when already cached", async () => {
    ...
    const { wrapper } = mountWithApp(EditorMarkdown, {
      stores: {
        editor: (store) => {
          // -> Stands in for App.vue's own prefetch having already landed by the time this component
          //    mounts -- the normal case. Seeded and spied inside the seed callback, which
          //    `mountWithApp` runs before the component mounts.
