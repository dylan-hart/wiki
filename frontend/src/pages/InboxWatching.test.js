  ...
  function mockWatchedPage(extraGetHandler) {
    API_CLIENT.get.mockImplementation((url) => {
      if (url === 'sites/site-1/watching') {
        ...
        //    `WATCHED_PAGE` itself here would let one test's successful save leak into every test
        //    that runs after it in the same file.
        return {
          json: () =>
            Promise.resolve([{ ...WATCHED_PAGE, preference: { ...WATCHED_PAGE.preference } }])
        }
      }
      if (extraGetHandler) {
  ...
  it('opens seeded from the page’s current preference, and saving PATCHes only that watch', async () => {
    ...
    API_CLIENT.patch.mockReturnValueOnce({
      json: () =>
        Promise.resolve({
          ok: true,
          preference: { ...WATCHED_PAGE.preference, notifyOnMoved: false }
        })
    })
