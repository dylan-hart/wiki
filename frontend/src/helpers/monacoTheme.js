 ...
 * re-resolving a variable the way every other surface does. It has to be applied by registering a
 * second theme and SWITCHING to it.
 *
 * Each surface that mounts Monaco registers its own ground, so rather than hand-writing a Cobalt
 * twin of each, the pair is derived: every colour a Ledger theme names is looked up in the table
 * below and replaced with Cobalt's answer for the same ROLE, so a surface keeps its own shape while
 * moving onto the other aesthetic's ramp. A colour with no entry is passed through unchanged, which
 * is correct for the ones that are not aesthetic at all (a diff's red and green, `#fff`).
 *
...
/**
 * Ledger tone -> Cobalt tone, by the role each plays in an editor surface: grounds onto Cobalt's own
 * dark ramp (`#141c4f` card, `#0b1238` the well a gutter sits in, `#1c2a70` the raised rung a line
 * highlight uses), and the accent pair keeping the fill/text split (`#ff4d5a` untexted, `#c8303c`
 * under a label).
 */
const COBALT_TONE = {
  // -- Grounds
 ...
 *   The surface's own Ledger theme.
 */
