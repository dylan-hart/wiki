/**
 * Writing a table and reading one back live together because they have to agree on one shape: two
 * definitions of "what a table looks like" would mean a table that came back out of the editor
 * differing from the one that went in, in whitespace nobody asked to change.
 *
 ...
 * above it -- so one renders as a table only while the site has the MultiMarkdown Table option on.
 */
 ...
 * the content stylesheet styles it by.
 */
const ATTRS_LINE = /^ {0,3}\{([^}]*)\}\s*$/

/**
 * A cell's own `|` is escaped, and a newline -- which only a paste can produce -- becomes a space,
 * because there is no way to write either into a table row.
 */
 ...
 *
 * `otherAttrs` is whatever the author had in the `markdown-it-attrs` line that is not a class -- an
 * `#id`, say -- kept verbatim so that reading a table and writing it back does not quietly drop half
 * of its attributes.
 */
...
/**
 * Raw, unsplit and untrimmed, so `isCompact` can measure the whitespace. An escaped `\|` is not a
 * separator, and the outer pipes markdown allows on each side leave an empty segment at each end
...
/**
 * Also what tells a table's header row from a paragraph that happens to hold a pipe: a table is a
 * line followed by one of these.
 *
...
/**
 * Keeps reading a table and writing it back from reformatting it on the author's behalf.
 *
 ...
 * `buildTable` emits exactly. Padding is a cell holding spaces beyond the single one that keeps the
 * text off the pipe -- and, in a delimiter row, a run of dashes longer than a compact table needs.
 */
...
export function parseTable(source) {
  ...
  /*
    ...
    only line of the block that is not one.
  */
 ...
 *
 * Everything else that can sit under a table is far more common than a second body, so the reading is
 * deliberately narrow: one blank line, then a row that opens with a pipe, carries this table's number
 * of columns, is not a delimiter row (that would start a headerless table of its own) and is not
 * followed by one (that would make it the header of an ordinary one).
 */
 ...
 *
 * Deliberately left out: a table inside a fenced block, which is a code sample and not a table, and a
 * table using a multi-line cell, a `^^` rowspan or a second body. The editor's model has no place to
 * keep any of those, so offering to edit one would be offering to throw it away.
 */
export function findEditableTables(text) {
  ...
  linesOutsideFences(lines, (line, index) => {
    ...
    /*
      ...
      with the delimiter row under it. An ordinary table's delimiter row is never mistaken for the first
      kind, because the walk skips past every line of a block it has already been through.
    */
    ...
    /*
      ...
      leaving it out of the range would have an update write a second one under the first.
    */
