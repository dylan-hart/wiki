...
/*
  Both variants built once, keyed by `timeFormat`, rather than one per `formatTimePart()` call.
  `hourCycle` rather than `hour12: false`, which some locales render as 24:00 where they mean 00:00.
*/
const timeFormats = {
...
function formatTimePart(zoned, timeFormat) {
  return timeFormats[timeFormat === '24h' ? '24h' : '12h'].format(zoned.toPlainDateTime())
}
