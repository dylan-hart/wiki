...
describe('block-plantuml', () => {
  ...
  /*
    `block-plantuml` encodes diagram source straight into a GET URL with no POST fallback. A
    diagram large enough to push that URL past what a reverse proxy will accept used to fail
    silently as a generic broken-image message via `_explain()`, once the
