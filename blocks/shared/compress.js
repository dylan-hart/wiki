 ...
 * `pako` dependency `block-kroki` (`deflate`, zlib header) and `block-plantuml` (`deflateRaw`, no
 * header) used for the same job -- packing a diagram source into a GET URL. Every browser able to
 * run these Lit blocks implements `CompressionStream`, including both `'deflate'` and
 * `'deflate-raw'`.
 *
