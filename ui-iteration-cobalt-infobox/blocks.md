...
Cobalt: card `1px #c9d6fb` (blue hairline, not the `#dfe5f5` card border — infobox only) 8px, `overflow:hidden`; dark `rgba(143,176,255,.28)`. Full Cobalt note in `infobox-cobalt.md`. `.name` band `#1f4fd6` (dark `#3d6df7`), Barlow Condensed 600 15px white. No rules: rows alternate `#fff` / `#f2f5ff` (`dl > :nth-child(4n+3), :nth-child(4n+4)`), `.group` on `#e6edff` Barlow 600 12px `#1f4fd6`, no group-end rule. Check `#16795d`, x `#c8303c`, links `#1f4fd6`. Image well `#e6edff` 6px.
